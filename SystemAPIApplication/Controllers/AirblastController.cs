    ...
    public class AirblastController : ControllerBase
    {
        ...
        public IActionResult AirblastMerge()
        {
            ...
            foreach (BsonDocument fireball in result)
            {
                ...
                if (geom == null)
                    geom = _geometryAnalysisService.GetShockWaveGeometry(lon, lat, yield, alt);
                else
                    geom = geom.Union(_geometryAnalysisService.GetShockWaveGeometry(lon, lat, yield, alt));
            }
        ...
        public IActionResult AirblastMulti()
        {
            ...
            foreach (BsonDocument fireball in result)
            {
                ...
                double r = _geometryAnalysisService.GetShockWaveRadius(yield / 1000, alt * 3.2808399);
